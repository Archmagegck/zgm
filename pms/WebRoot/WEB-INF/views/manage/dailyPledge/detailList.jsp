...
<html>
	...
	<body>
		<div style="width: 90%">
		<form action="${ctx }/manage/dailyPledge/${pledgeRecord.id }/updateInput" method="get" id="myForm" name="myForm">
			...
			<div style="line-height:200%;font-family:宋体;font-size:14px" id="pcontent">
				致：
				<u>${pledgeRecord.delegator.name }</u>
				（质权人）<br/>
				&nbsp;&nbsp;&nbsp;&nbsp;本公司已按照相关协议于
				<input type="text" name="iyear" value="${pledgeRecord.recYear }" size="5" class="{required:true,number:true}"/>年
				<input type="text" name="imonth" value="${pledgeRecord.recMonth }" size="5" class="{required:true,number:true}"/>月
				<input type="text" name="iday" value="${pledgeRecord.recDay }" size="5" class="{required:true,number:true}"/>日
				接收下表货物并开始履行监管责任。<br/>
				本公司了解，出质人已将下表所列货物质押给贵行，并对质物的真实性、合法性负责。
				本公司业已收到贵行与出质人共同签发的编号为
				<input type="text" name="preCode" value="${pledgeRecord.preCode }" size="20" class="required"/>
				的《质物种类、价格、最低要求通知书（代出质通知书）》，
				本公司同意接受贵行委托并将按照贵行的指示代为监管质物。
				本公司确认上述质物（详见下表）已存放于我公司拥有使用权的仓库/场地，上述质物确已在本公司的占有、保管、监管之下。
				本公司将严格按照编号为
				<input type="text" name="proCode" value="${pledgeRecord.proCode }" size="20" class="required"/>
				的《黄金产业链融资项下黄金制品质押监管协议》的规定履行占有、保管、监管责任。
				本质物清单为编号为
				<u>${pledgeRecord.code }</u>
