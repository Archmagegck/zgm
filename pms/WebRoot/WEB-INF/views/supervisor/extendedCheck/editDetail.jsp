...
<html>
  <head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#checkMethod").val('${extendedCheckDetail.checkMethod}');
			$("#checkResult").val('${extendedCheckDetail.checkResult}');
		});
  ...
  <body>
    <form id="myForm" name="myForm" action="${ctx}/supervisor/extendedCheck/updateDetail/${index}" method="post">
    	<div id="content">
						...
						<td width="70%">
							<select id="style" name="style.id" class="required" onchange="changeStyle()">
								<c:forEach items="${styleList}" var="style">
									<c:choose>
										<c:when test="${style.id == extendedCheckDetail.style.id }">
							...
							<input type="hidden" id="styleName" name="style.name" value="${extendedCheckDetail.style.name}">
						</td>
					</tr>
					<tr>
						<td width="30%">
