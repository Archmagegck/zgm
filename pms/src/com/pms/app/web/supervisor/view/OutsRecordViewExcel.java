...
public class OutsRecordViewExcel extends AbstractExcelView {

	@SuppressWarnings("unchecked")
	@Override
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setContentType(ServletUtils.EXCEL_TYPE);
		ServletUtils.setFileDownloadHeader(response, "出库单列表.xls");
		
		List<OutsRecord> outsRecordList = (List<OutsRecord>) model.get("outsRecordList");
		Date date = (Date) model.get("date");
		
		String[] title = { "序号", "出库单号", "总重量（g）", "出库时间", "备注", "审核状态"};
		int length = title.length - 1;
		
		Sheet s = workbook.createSheet("出库单");
		
		for (int i = 0; i < title.length; i++) {
			s.setColumnWidth(i, 28 * 256);// 设置列的宽度
		}
		...
		for(OutsRecord outsRecord : outsRecordList) {
			...
			c = r.createCell(1);
			c.setCellStyle(cOtherCellStyle);
			c.setCellValue(outsRecord.getCode());
			
			c = r.createCell(2);
			c.setCellStyle(numCellStyle);
			c.setCellValue(outsRecord.getSumWeight());
			sumWeight += outsRecord.getSumWeight();
			
			c = r.createCell(3);
			c.setCellStyle(cOtherCellStyle);
			c.setCellValue(new DateTime(outsRecord.getDate()).toString("yyyy-MM-dd hh:mm:ss"));
			
			c = r.createCell(4);
			c.setCellStyle(cOtherCellStyle);
			c.setCellValue(outsRecord.getDesc());
			
			c = r.createCell(5);
			c.setCellStyle(cOtherCellStyle);
			c.setCellValue(outsRecord.getAuditState().getTitle());
